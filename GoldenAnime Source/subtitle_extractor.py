...
def example_usage():
    ...
    # Recherche pour un film en français
    results = extractor.search("tt0111161", lang_filter="French")

    print(f"--- Résultats pour 'The Shawshank Redemption' (FR) ---")
    for sub in results:
        print(f"[{sub['source']}] {sub['lang']}: {sub['url']}")


if __name__ == "__main__":
    # Support CLI maintenu pour les tests rapides
    parser = argparse.ArgumentParser(
        description="Extract subtitles from CineStream sources."
    )
    parser.add_argument("imdb_id", help="IMDB ID (e.g., tt0111161)")
    parser.add_argument("-s", "--season", type=int, help="Season number")
    parser.add_argument("-e", "--episode", type=int, help="Episode number")
    parser.add_argument("-l", "--lang", help="Language filter (ex: fr, French, en)")

    args = parser.parse_args()

    extractor = SubtitleExtractor()
    results = extractor.search(args.imdb_id, args.season, args.episode, args.lang)

    if not results:
        print("Aucun sous-titre trouvé.")
    else:
        for sub in results:
            print(f"{sub['source']:<25} | {sub.get('lang'):<15} | {sub['url']}")
